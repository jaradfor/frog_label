...
Choose `GRE`, select Draw Box, and drag over a call in the spectrogram. The `?` button opens isolated practice. Number keys `1`–`4` toggle Species, Details, Display, and Dataset when an editable control does not own the key.
