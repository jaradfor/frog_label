...
function AppContent({ config, onLogout }) {
  ...
  const { currentTask, selectedAudio, boxes, setBoxes, submitAnnotation } =
    useAnnotationSession(config);
      ...
      {!currentTask && (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <p style={{ color: theme.text }} className="font-display text-2xl mb-4">
              No Tasks Available
            </p>
            <p style={{ color: theme.text }} className="font-display text-sm">
              All tasks have been completed or there are no tasks to annotate at this time.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="flex gap-2 px-2 py-2 flex-1 min-h-0 overflow-hidden items-stretch">
          {showLeftPanel && (
            <div
              ...
              className="w-48 shrink-0 rounded-xl p-2 overflow-y-auto"
            >
          ...
          <div className="flex-1 min-w-0 min-h-0 flex flex-col relative">
            <div
              style={{ backgroundColor: theme.panels }}
              className="p-2 rounded-xl flex flex-wrap justify-center items-center gap-2"
            >
              <SpectrogramControls
                zoomX={zoomX}
                setZoomX={setZoomX}
            ...
            <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
              <WaveformSpectrogram
                ...
                currTool={currTool}
              />
              <Tools theme={theme} lightMode={lightMode} />
            </div>
