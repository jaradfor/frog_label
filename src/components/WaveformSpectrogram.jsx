...
function WaveformSpectrogram({
  ...
  useEffect(() => {
    ...
    getAudioInfo(selectedAudio)
      .then(({ sampleRate, maxFrequency }) => {
        if (cancelled) return;
        ...
        fetchAuthenticatedAudio(selectedAudio)
          .then((url) => {
            blobUrl = url;
  ...
  return (
    <div
      ...
      className="flex-1 min-h-0 p-6 pb-2 rounded-xl my-2 overflow-auto"
    >
      <div className="flex">
        {/* Frequency Labels */}
        <div
        ...
        >
          {localSampleRate && lowCutoff != null && highCutoff != null && highCutoff > 0 ? (
            ...
            <span
              className="absolute right-1 text-[12px] font-display"
