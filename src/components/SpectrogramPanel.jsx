  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex justify-between items-center gap-2">
        <span className="font-display text-sm" style={{ color: theme.text }}>
          {label}
        </span>
        <div className="flex items-center gap-1">
          ...
          />
          {unit && (
            <span className="font-display text-xs" style={{ color: theme.text }}>
              {unit}
            </span>
          )}
        </div>
      ...
      <input
        ...
        value={value}
        onChange={(e) =>
          onChange(decimals === 0 ? Number(e.target.value) : parseFloat(e.target.value))
        }
...
function SpectrogramPanel({ theme }) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setPendingLow(lowCutoff);
  }, [lowCutoff]);
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setPendingHigh(highCutoff);
  }, [highCutoff]);

  const colorScales = [
    {
      name: 'roseus',
      gradient: 'linear-gradient(to right, #dbc642, #f36e1c, #d94f8a, #7a1b6c, #2b0a3d )',
      dbfs: [-120, -90, -60, -30, 0],
      colorMap: 'roseus',
    },
    {
    ...
    {
      name: 'viridis',
      gradient: 'linear-gradient(to right, #440154, #30678d, #35b778, #fde724)',
      dbfs: [-120, -90, -60, -30, 0],
      colorMap: generateViridis(),
    },
    {
      name: 'magma',
      gradient: 'linear-gradient(to right, #000004, #51127c, #b73779, #fc8961, #fbfdbf)',
      dbfs: [-120, -90, -60, -30, 0],
      colorMap: generateMagma(),
    },
    {
      name: 'plasma',
      gradient: 'linear-gradient(to right, #0d0887, #6a00a8, #b12a90, #e16462, #fca636, #f0f921)',
      dbfs: [-120, -90, -60, -30, 0],
      colorMap: generatePlasma(),
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        ...
        className="flex flex-col flex-1 rounded-lg p-2 gap-1"
      >
        <span className={titleClass} style={{ color: theme.text }}>
          FFT Window Size
        </span>
      ...
      <div
        style={{ backgroundColor: theme.group, color: theme.text }}
        className="flex flex-col flex-1 rounded-lg p-2 gap-3"
      >
        <span className={titleClass} style={{ color: theme.text }}>
          Frame Overlap
        </span>
      ...
      >
        <span className={titleClass} style={{ color: theme.text }}>
          Colormap & Level Scale
        </span>
        <div className="flex flex-col gap-2">
          <div className="flex flex-col gap-1">
            <span className={labelClass} style={{ color: theme.text }}>
              Palette
            </span>
            {colorScales.map(({ name, gradient }) => (
              ...
              >
                <div
                  style={{ borderColor: theme.keyButtons }}
                  className="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0"
                >
                  {colorScale === name && (
                    <div
                      style={{ backgroundColor: theme.keyButtons }}
                      className="w-2 h-2 rounded-full"
                    />
                  )}
                </div>
                <div
                  className="h-4 rounded-sm flex-1 capitalize"
                  style={{ background: gradient }}
                />
                <span className={`${metaClass} capitalize`} style={{ color: theme.text }}>
                  {name}
                </span>
              </div>
            ))}
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <div className="flex flex-row justify-between">
              {colorName.dbfs.map((val) => (
                <span key={val} className={metaClass} style={{ color: theme.text }}>
                  {val}
                </span>
              ))}
            </div>
