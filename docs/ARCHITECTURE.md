...
Active V2 codes are 1–6 uppercase letters from `QWERTASDFGZXCVB` and are unique within a project. `selectionPriority` is an administrator-set integer from 0 to 1,000,000 and affects ambiguous prefix selection. Newly written V2 annotation snapshots retain it so annotation-scoped Enterprise species keep the same ordering after reload; the field remains optional when reading earlier V2 documents. Full Species Name is required. `speciesId` is immutable, while code/name/priority are current mutable catalog fields. A V2 catalog's optional `historicalSpecies` lane contains validated V1 records for display only; those records are never offered to the Space prefix index, even when an old code happens to use left-hand letters. V1 documents remain readable and are upgraded in memory; historical box snapshots retain codes such as `PER` and `COR` unchanged.
...
One `AudioResource` fetches/decodes a task once and preserves native stereo playback. Scientific analysis is mono and selectable as Average energy, Max, Left, or Right. Complete overlapping STFT windows run in a task-scoped Blob worker when available and in a cancellable cooperative executor otherwise. The two paths share the same algorithm. Defaults are a ~20 ms target duration expanded to the next supported power-of-two FFT size, a Hann window, and 75% overlap. Users may select a 10/20/40/80 ms target duration, Hann/Hamming/Blackman/rectangular window, and 0/25/50/75% overlap. Changing any of those analysis settings rebuilds the task analysis and its dependent tiles. The bounded POC accepts at most two channels, 192 kHz source rate, five minutes, 128 MiB, and 30 million decoded channel-samples.

Exact replacement frames are partitioned into palette-independent 256×256 dB tiles and assembled offscreen before one atomic Canvas2D front-surface commit. WebGL2 stores those tiles in one slot-based `R32F` atlas capped at 48 MiB and applies palette, brightness, contrast, and the selected −120 to −40 dBFS display floor through a 256-entry lookup texture; the fallback retains the same dB tiles and colorizes cooperatively in Canvas2D. The CPU tile LRU is independently byte-capped at 48 MiB. Visible misses run first, followed by best-effort adjacent center-zoom prefetch, and all background work is preemptible by a newer view generation. Out-of-grid rings are deliberately omitted because an exact view-grid key can never reuse them after a pan.

The authoritative tiles are deliberately keyed to the exact viewport grid: audio generation, channel mode, frequency scale and adjustable emphasis, Q/E zoom level, full IEEE-754 view bounds, raster dimensions, and tile coordinates. Analysis-setting changes advance the analysis generation before rebuilding, so tiles cannot cross window or overlap configurations; the exact viewport-grid key and pooling rules otherwise remain unchanged. A fixed pooled world raster cannot preserve FrogLabel's peak-pooling result at arbitrary fractional pan offsets because nearly every output pixel's half-open frame/bin rectangle changes. Each view tile is therefore computed using its global pixel offset and the full raster dimensions, which makes seams and arbitrary offsets bit-for-bit equivalent to monolithic pooling. Overscan and adjacent-zoom tiles are only best-effort warmups; retained-frame reprojection supplies immediate feedback when a new fractional view has a different exact key. Palette, brightness, contrast, and display floor are excluded from the scientific key and never rerun pooling.

The Vite development virtual-worker plugin watches the worker bundle's transitive inputs and invalidates its generated module when any input changes. Local development therefore cannot retain stale spectrogram-worker logic after a source edit; production still emits one deterministic bundled worker.

Frequency projection is centralized in `domain/frequencyScale.ts`. Linear and logarithmic retain their conventional transforms. Adjustable mode uses an exactly invertible power-law curve whose exponent moves continuously from 1.0 to 0.25 as low-frequency emphasis moves from 0% to 100%. The same forward/inverse functions drive raster row pooling, annotation projection, pointer geometry, axis labels, camera pan/zoom, retained-frame placement, worker caches, and tile keys. Canonical annotations never store display-scale state; their frequency bounds remain hertz.
