...
3. Drag around a call. Press `T` to switch between Draw and Select/Resize. Keep the mouse in your right hand: `WASD` pans; `Q`/`E` zooms both axes; `Shift+Q/E` zooms time only; `Shift+W/S` zooms frequency only; `X` fits; and middle/right drag pans from either tool.
